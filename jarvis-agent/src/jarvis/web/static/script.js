...
class TTSPlayer {
     ...
     * playback queue.
     *
     * @param {string} base64Audio - The base64 string of audio data.
     ...
     */
    enqueue(base64Audio, mimeType = 'audio/mp3') {
        if (!this.enabled || this.stopped) return;
        this.queue.push({ b64: base64Audio, mime: mimeType || 'audio/mp3' });
    ...
    async _playLoop() {
        ...
        while (this.queue.length > 0) {
            ...
            try {
                await this._playB64(item);                      // Wait for it to finish playing
            } catch (e) {
    ...
    /**
     * _playB64(item) — Plays a single base64-encoded audio chunk.
     *
     * @param {{b64: string, mime: string}|string} item
    ...
    _playB64(item) {
        return new Promise(resolve => {
            const payload = typeof item === 'string' ? { b64: item, mime: 'audio/mp3' } : item;
            const mime = payload.mime || 'audio/mp3';
            this.audio.src = `data:${mime};base64,` + payload.b64;
            const done = () => { resolve(); };
...
function renderSearchResults(payload) {
    ...
    const results = payload.results || payload.sources || [];
        ...
        while (true) {
            ...
            for (const line of lines) {
                ...
                try {
                    ...
                    const audioMime = data.audio_mime || 'audio/mp3';
                    if (audioB64 && ttsPlayer) {
                        ttsPlayer.enqueue(audioB64, audioMime);
                    }
